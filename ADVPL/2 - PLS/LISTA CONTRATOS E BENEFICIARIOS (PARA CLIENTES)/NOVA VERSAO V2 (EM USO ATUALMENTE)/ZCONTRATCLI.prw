...
Static Function PegaDados(cSA1cliente, cSA1lojacli, cTipCli)
    ...
    ELSE
            ...
            cQuery += " GROUP BY B.BA3_FILIAL,B.BA3_XCARTE,E.BT5_NOME,F.BQL_DESCRI,G.ZI0_DESCRI,B.BA3_MOTBLO,B.BA3_DATBLO,B.BA3_DATBAS,H.A3_NOME,B.BA3_CODINT,B.BA3_CODEMP,B.BA3_CONEMP,B.BA3_VERCON,B.BA3_SUBCON,B.BA3_VERSUB,B.BA3_MATEMP "
...
Static Function MontaTela(cSA1cliente, cSA1lojacli, cTipCli)
    ...
    DEFINE DIALOG oDlg TITLE "Contratos Cliente" FROM aCoors[1], aCoors[2] TO aCoors[3]/1.76 , aCoors[4] - (aCoors[4]/3)PIXEL
        ...
        While (_cAlias)->(!Eof())
            ...
            aAdd(aBrowse, { IF((_cAlias)->STATUSC = 'ATIVO',.T.,.F.),(_cAlias)->FILIAL,(_cAlias)->GRUPOEMP,(_cAlias)->IDCONTRATO,(_cAlias)->NUMERO,(_cAlias)->PERFIL,(_cAlias)->FORMPAG,(_cAlias)->CONDPAG,(_cAlias)->STATUSC,(_cAlias)->DTBASE,(_cAlias)->VENDEDOR,(_cAlias)->FILIALCTR,(_cAlias)->CODINT,(_cAlias)->CODEMP,(_cAlias)->CONEMP,(_cAlias)->VERCON,(_cAlias)->SUBCON,(_cAlias)->VERSUB,(_cAlias)->MATEMP }) // DADOS DA QUERY
        ...
        // Cria Browse
        oBrowse := TCBrowse():New( aCoors[1]+25 , aCoors[1], aCoors[3] - (aCoors[3]/2.75), aCoors[3]/4 ,, {'','Filial','Grupo Empresa','Id Contrato','Numero','Perfil','Form. Pg.','Cond.Pg.','Status','Dt. Base','Vendedor'},{20,50,50,50,50,50,50,50,50,50}, oDlg,,,,,{||},,,,,,,.F.,,.T.,,.F.,,, )
        ...
        oBrowse:bLine := {||{ If(aBrowse[oBrowse:nAt,01],oOK,oNO),aBrowse[oBrowse:nAt,02],aBrowse[oBrowse:nAt,03],aBrowse[oBrowse:nAt,04],aBrowse[oBrowse:nAt,05],aBrowse[oBrowse:nAt,06],aBrowse[oBrowse:nAt,07],aBrowse[oBrowse:nAt,08],aBrowse[oBrowse:nAt,09],aBrowse[oBrowse:nAt,10],aBrowse[oBrowse:nAt,11] }}
        ...
        TButton():New( aCoors[1]+10, aCoors[3] - (aCoors[3]/2.77), "Resumo Contrato",   oDlg,{|| MsAguarde({|| ResumoCtr( aBrowse[oBrowse:nAt,12],aBrowse[oBrowse:nAt,13],aBrowse[oBrowse:nAt,14],aBrowse[oBrowse:nAt,15],aBrowse[oBrowse:nAt,16],aBrowse[oBrowse:nAt,17],aBrowse[oBrowse:nAt,18],aBrowse[oBrowse:nAt,19] )  },"Aguarde","Buscando Dados do Contrato...")  },50,018,,,.F.,.T.,.F.,,.F.,,,.F. )
        TButton():New( aCoors[1]+35, aCoors[3] - (aCoors[3]/2.77), "Beneficiarios ",    oDlg,{|| MsAguarde({|| BenefiCtr( aBrowse[oBrowse:nAt,12],aBrowse[oBrowse:nAt,13],aBrowse[oBrowse:nAt,14],aBrowse[oBrowse:nAt,15],aBrowse[oBrowse:nAt,16],aBrowse[oBrowse:nAt,17],aBrowse[oBrowse:nAt,18],aBrowse[oBrowse:nAt,19] )  },"Aguarde","Buscando Beneficiarios...")  },50,018,,,.F.,.T.,.F.,,.F.,,,.F. )
...
Static Function BenefiCtr(cFilialCtr,cCodint,cCodemp,cConemp,cVercon,cSubcon,cVersub,cMatemp)
    ...
    aCoors := FWGetDialogSize()
        ...
        ENDIF
        cQueryBenefi += " GROUP BY B.BA1_XCARTE,B.BA1_NOMUSR,B.BA1_CPFUSR,B.BA1_ZATEND,B.BA1_DATBLO,B.BA1_MOTBLO,B.BA1_TIPUSU "
    ...
    Else
        ...
        DEFINE DIALOG oDlg TITLE "Beneficiarios - Contrato Medicar " FROM aCoors[1], aCoors[2] TO aCoors[3] - (aCoors[3]/4), aCoors[4] - (aCoors[4]/3) PIXEL
            ...
            While (AliasBeneCtr)->(!Eof())
                ...
                aAdd(aGrade, { IF((AliasBeneCtr)->STATUSB = 'ATIVO',.T.,.F.),(AliasBeneCtr)->CARTEIRA,(AliasBeneCtr)->BENEFI,(AliasBeneCtr)->CPF,(AliasBeneCtr)->ATEND,(AliasBeneCtr)->STATUSB,(AliasBeneCtr)->VALOR,(AliasBeneCtr)->TIPO }) // DADOS DA GRADE
            ...
            oBrowse := TCBrowse():New( aCoors[1]+30 , aCoors[1], aCoors[3] - (aCoors[3]/2.75), aCoors[3]/3,, {'','Carteira','Beneficiario','Cpf','Tem Atend.','Status','Valor','Tipo'},{20,50,50,50,50,50,50,50}, oDlg,,,,,{||},,,,,,,.F.,"aGrade",.T.,,.F.,,, ) //CABECARIO DA GRADE
            ...
            TButton():New( aCoors[1]+35, aCoors[3] - (aCoors[3]/2.77) , "Voltar"            , oDlg, {|| oDlg:End() }, 50,018, ,,,.T.,,,,,,)
...
Static Function fPesquisa(cFilialCtr,cCodint,cCodemp,cConemp,cVercon,cSubcon,cVersub,cMatemp,cPesq,cOpc,aGrade,oBrowse)
    ...
    aCoors := FWGetDialogSize()
        ...
        Endif
        cQueryAtt += " GROUP BY B.BA1_XCARTE,B.BA1_NOMUSR,B.BA1_CPFUSR,B.BA1_ZATEND,B.BA1_DATBLO,B.BA1_MOTBLO,B.BA1_TIPUSU "
    ...
    Else
        ...
        While (AliasAttBene)->(!Eof())
            ...
            aAdd(aGradeTT, { IF((AliasAttBene)->STATUSB = 'ATIVO',.T.,.F.),(AliasAttBene)->CARTEIRA,(AliasAttBene)->BENEFI,(AliasAttBene)->CPF,(AliasAttBene)->ATEND,(AliasAttBene)->STATUSB,(AliasAttBene)->VALOR,(AliasAttBene)->TIPO }) // DADOS DA GRADE
        
            (AliasAttBene)->(dBskip())

        EndDo

        oBrowse := TCBrowse():New( aCoors[1]+30 , aCoors[1], aCoors[3] - (aCoors[3]/2.75), aCoors[3]/3,, {'','Carteira','Beneficiario','Cpf','Tem Atend.','Status','Valor','Tipo'},{20,50,50,50,50,50,50,50}, oDlg,,,,,{||},,,,,,,.F.,,.T.,,.F.,,, ) //CABECARIO DA GRADE
        oBrowse:SetArray(aGradeTT)
        oBrowse:bLine := {||{ If(aGradeTT[oBrowse:nAt,01],oOK,oNO),aGradeTT[oBrowse:nAt,02],aGradeTT[oBrowse:nAt,03],aGradeTT[oBrowse:nAt,04],aGradeTT[oBrowse:nAt,05],aGradeTT[oBrowse:nAt,06],aGradeTT[oBrowse:nAt,07],aGradeTT[oBrowse:nAt,08] }} //EXIBICAO DA GRADE
