...
RELATORIO CADASTRO DE FORNECEDORES SEM DADOS BANCARIOS CADASTRADOS.

*/


User Function ZRFORNBANC()
...
Static Function fMontaExcel(cPasta)
	...
	oExcel:Activate()
	cArqBem := cPasta + '\' + 'FORNECEDORES_SEM_DADOS_BANCARIOS' + SubStr( DTOC(Date()),1,2 ) + SubStr( DTOC(Date()),4,2 ) + SubStr( DTOC(Date()),7,4 ) + '.xlsx'
