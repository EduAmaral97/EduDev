...
User function ZCTRMED()
    ...
    cQuery += "END AS VALOR, "
    cQuery += " CASE  "
    cQuery += "     WHEN B.BA3_IDBENN <> '' THEN '1'  "
    cQuery += "     ELSE '0'  "
    cQuery += " END             AS IDENT,  "
    cQuery += " CASE  "
    cQuery += "     WHEN B.BA3_MOTBLO <> '' THEN 'BLOQUEADO'  "
    cQuery += "     ELSE 'ATIVO'  "
    cQuery += " END AS STATUSC,  "
    ...
    cQuery += " LEFT JOIN BA1010 C ON C.D_E_L_E_T_ = '' AND C.BA1_FILIAL = B.BA3_FILIAL AND C.BA1_CODINT = B.BA3_CODINT AND C.BA1_CODEMP = B.BA3_CODEMP AND C.BA1_CONEMP = B.BA3_CONEMP AND C.BA1_SUBCON = B.BA3_SUBCON AND C.BA1_MATEMP = B.BA3_MATEMP AND C.BA1_MATRIC = B.BA3_MATRIC  "
    cQuery += " LEFT JOIN BDK010 D ON D.D_E_L_E_T_ = '' AND D.BDK_FILIAL = C.BA1_FILIAL AND D.BDK_CODINT = C.BA1_CODINT AND D.BDK_CODEMP = C.BA1_CODEMP AND D.BDK_MATRIC = C.BA1_MATRIC AND D.BDK_TIPREG = C.BA1_TIPREG   "
    cQuery += " LEFT JOIN BT5010 E ON E.D_E_L_E_T_ = '' AND E.BT5_FILIAL = A.BQC_FILIAL AND E.BT5_CODINT = A.BQC_CODINT AND E.BT5_CODIGO = A.BQC_CODEMP AND E.BT5_NUMCON = A.BQC_NUMCON AND E.BT5_VERSAO = A.BQC_VERCON  "
    ...
    cQuery += " GROUP BY C.BA1_FILIAL,B.BA3_IDBENN,B.BA3_MATEMP,B.BA3_XCARTE,E.BT5_NOME,F.BQL_DESCRI,G.ZI0_DESCRI, B.BA3_MOTBLO, B.BA3_DATBLO, B.BA3_DATBAS, H.A3_NOME,B.BA3_CODINT,B.BA3_CODEMP,B.BA3_CONEMP,B.BA3_VERCON,B.BA3_VERSUB,C.BA1_DATBLO  "
    ...
    if TCSqlExec(cQuery) >= 0
        ...
        aAdd(aBrowse, {"Qtd Vidas",     "QTDVIDAS",     "N",10                  ,0,"@E 999999999"})
        aAdd(aBrowse, {"Valor",         "VALOR",        "N",12                  ,2,"@E 999,999,999.99"})
        ...
        oBrw:AddButton( "Voltar"  ,{|| lRet := .T., oDlg:end() } ,, 2 )
        oBrw:AddButton( "Beneficiarios"  , {|| U_ZBENEFCTR(cSA1cliente, cSA1lojacli, cSA1tipCli,(oTable:GetAlias())->IDCONTRATO, (oTable:GetAlias())->CODINT,(oTable:GetAlias())->CODEMP, (oTable:GetAlias())->NUMCOM, (oTable:GetAlias())->VERCON,(oTable:GetAlias())->VERSUB,(oTable:GetAlias())->IDENT) } ,, 3 )         
        oBrw:AddButton( "Resumo Contrato"  , {|| oDlg:end() } ,, 4 ) 
        ...
        oBrw:AddLegend( "STATUSC = 'ATIVO' ", "GREEN",  "Registro Ativo" )
        oBrw:AddLegend( "STATUSC = 'BLOQUEADO' ", "RED",    "Registro Bloqueado" )
