...
INSERT INTO #AUX (AUX_DATSLD, AUX_MES, AUX_ANO, AUX_CODPROD, AUX_PRODUTO, AUX_UND, AUX_CODFOR, AUX_LOJFOR, AUX_GRUPPROD, AUX_CODLIN, AUX_CODFAM, AUX_CUST_INI, AUX_CUST_ATU, AUX_ULT_CUST, AUX_CUST_ATT, AUX_SALDO, AUX_CUSTO) 


SELECT @DTSLD AS DT_SLD, @MES AS MES_PROCESSADO, @ANO AS ANO_PROCESSADO, *,
(SELECT_PRINCIPAL.SALDO_FINAL * SELECT_PRINCIPAL.CUSTO_ATT) AS CUSTO_FINAL_ATT
FROM(
SELECT 
SALDO.COD_PROD,
SALDO.PRODUTO,
...
SELECT 	A.B1_COD   	  AS COD_PROD,
		...
		ISNULL(( SELECT SUM(D3_QUANT) FROM SD3010 WHERE D_E_L_E_T_ = '' AND D3_COD = A.B1_COD AND D3_TM >= 500 AND D3_EMISSAO BETWEEN @cDatIni AND @cDatFim),0) AS QTD_SAI_SD3,	
	 	ISNULL((SELECT TOP 1 B9_CUSTD FROM SB9010 WHERE D_E_L_E_T_ = '' AND B9_COD = A.B1_COD AND  YEAR(B9_DATA) = 2017 AND MONTH(B9_DATA) = 12),0) AS CUSTO_INI,									
