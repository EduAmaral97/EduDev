...
SELECT 	A.B1_COD   	  AS COD_PROD,
		...
		ISNULL(( SELECT SUM(B9_QINI)  FROM SB9010 WHERE D_E_L_E_T_ = @BRANCO AND B9_FILIAL = @FILIAL AND B9_COD = A.B1_COD AND YEAR(B9_DATA) = 2017),0) AS SLD_INI_SB9,
		ISNULL(( SELECT SUM(D1_QUANT) FROM SD1010 AS SD1, SF4010 AS SF4 WHERE SD1.D_E_L_E_T_ = @BRANCO AND SD1.D1_FILIAL = @FILIAL AND SF4.D_E_L_E_T_ = @BRANCO AND SD1.D1_COD = A.B1_COD AND SD1.D1_DTDIGIT BETWEEN @cDatIni AND @cDatFim AND SD1.D1_TES = SF4.F4_CODIGO AND SF4.F4_ESTOQUE = @F4ESTOQUE),0) AS QTD_ENT_SD1,
		ISNULL(( SELECT SUM(D2_QUANT) FROM SD2010 AS SD2, SF4010 AS SF4 WHERE SD2.D_E_L_E_T_ = @BRANCO AND SD2.D2_FILIAL = @FILIAL AND SF4.D_E_L_E_T_ = @BRANCO AND SD2.D2_COD = A.B1_COD AND SD2.D2_EMISSAO BETWEEN @cDatIni AND @cDatFim AND SD2.D2_TES = SF4.F4_CODIGO AND SF4.F4_ESTOQUE = @F4ESTOQUE),0) AS QTD_SAI_SD2,
		ISNULL(( SELECT SUM(D3_QUANT) FROM SD3010 WHERE D_E_L_E_T_ = @BRANCO AND D3_FILIAL = @FILIAL AND D3_COD = A.B1_COD AND D3_TM <= 499 AND D3_EMISSAO BETWEEN @cDatIni AND @cDatFim),0) AS QTD_ENT_SD3,
		ISNULL(( SELECT SUM(D3_QUANT) FROM SD3010 WHERE D_E_L_E_T_ = @BRANCO AND D3_FILIAL = @FILIAL AND D3_COD = A.B1_COD AND D3_TM >= 500 AND D3_EMISSAO BETWEEN @cDatIni AND @cDatFim),0) AS QTD_SAI_SD3,	
	 	ISNULL((SELECT TOP 1 B9_CUSTD FROM SB9010 WHERE D_E_L_E_T_ = @BRANCO AND B9_FILIAL = @FILIAL AND B9_COD = A.B1_COD AND  YEAR(B9_DATA) = 2017 AND MONTH(B9_DATA) = 12),0) AS CUSTO_INI,									
		...
		AND TAB1.ZZ2_FILIAL = @FILIAL
		AND TAB2.ZZ3_FILIAL = @FILIAL
...
AND B.D1_FILIAL = @FILIAL
AND C.D2_FILIAL = @FILIAL
AND D.D3_FILIAL = @FILIAL
AND E.B9_FILIAL = @FILIAL
AND A.B1_COD = @PROD 
