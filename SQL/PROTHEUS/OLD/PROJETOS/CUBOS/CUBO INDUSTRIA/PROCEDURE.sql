...
CREATE PROCEDURE Z_CUBO_IND AS


	EXEC UPDATE_ITEM_SL2_SC6_SC1
	DROP TABLE AUXZCUBOIND
	DROP TABLE ZCUBOIND
	
/*---------------------------------------------TABELA AUXILIAR--------------------------------------------------------*/


CREATE TABLE AUXZCUBOIND
    (
	Z_NUM_ATM			VARCHAR(6),
	...
	Z_QTD_PC			FLOAT,
	Z_EMI_PC			VARCHAR(30),
	Z_FILIAL_OP			VARCHAR(4),
	Z_NUM_OP			VARCHAR(6),
	Z_QTD_OP			FLOAT,
	Z_OP_FINALIZADO		VARCHAR(1),
	Z_DT_EMI_OP			VARCHAR(30),
	Z_DT_FINAL_OP		VARCHAR(30),	
	...
	Z_PRZ_ENT_CLI		INT,
	Z_DT_HJ				VARCHAR(30),
	Z_COMPRA_ATRASADA	VARCHAR(1),
    R_E_C_N_O_ 			INT IDENTITY(1,1),
	)
	
	
INSERT INTO AUXZCUBOIND
(
...
CASE 
	WHEN SC6A.C6_PRODUTO <> ''
THEN ISNULL((SELECT B1_GRUPO FROM SB1010 WHERE D_E_L_E_T_ = '' AND B1_COD = SC6A.C6_PRODUTO),0)
	WHEN SC6A.C6_PRODUTO IS NULL
THEN ISNULL((SELECT B1_GRUPO FROM SB1010 WHERE D_E_L_E_T_ = '' AND B1_COD = SB1.B1_COD),0)
END AS GRUPO_PROD,
...
SC2.C2_FILIAL			AS FILIAL_OP,
SC2.C2_NUM				AS NUM_OP,
SC2.C2_QUANT			AS QTD_OP,
CASE 
...
THEN 'S'
ELSE ''
END AS OP_FINALIZADO,
SC2.C2_EMISSAO			AS DT_EMI_OP,
SD3OP.D3_EMISSAO  		AS DT_FINAL_OP,
...
/*SC2 OP*/		LEFT JOIN SC2010 SC2   ON SC2.D_E_L_E_T_ = ''  AND SC2.C2_ZFILATM = SUA.UA_FILIAL AND SC2.C2_ZNUMATM = SUA.UA_NUM AND SUB.UB_PRODUTO = SC2.C2_PRODUTO
/*SD3 (OP)*/	LEFT JOIN SD3010 SD3OP ON SD3OP.D_E_L_E_T_ = '' AND SD3OP.D3_OP = SC2.C2_OP AND SD3OP.D3_COD = SC2.C2_PRODUTO
...
AND SUBSTRING(SB1.B1_GRUPO,1,2) IN ('CF','MR')
