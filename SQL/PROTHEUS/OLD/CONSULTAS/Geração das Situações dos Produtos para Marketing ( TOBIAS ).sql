...
SELECT 
A.B1_COD		AS CODPROD,
A.B1_ZIDWEB		AS IDWEB, 
A.B1_GRUPO		AS GRUPO, 
A.B1_DESC		AS DESC_PROD,
A.B1_ZDESRDZ	AS PRODUTO,
B5_ALTURA		AS ALTURA, 
B5_COMPR		AS COMPRIMENTO,	 
B5_LARG			AS LARGHURA,
A.B1_ZAPRES1	AS COD_APRE,
G.A5_CODPRF 	AS COD_PROD_FOR,
D.ZB2_DESFAM	AS FAMILIA,
A.B1_PROC		AS COD_FOR,
A.B1_LOJPROC	AS LOJA_FOR,
E.A2_NOME		AS FORNECEDOR,
A.B1_GRTRIB		AS GRUPO_TRIB,
A.B1_ZPROMOC	AS PROMOCAO,
A.B1_ZPERDES	AS FATOR_DESCONTO,
A.B1_ZPROWEB	AS PRODMOCAO_WEB,
A.B1_ZPDEWEB	AS FATOR_DESCONTO_WEB,
A.B1_ZUPSITE	AS UPSITE,
A.B1_ZMIXFL1 AS MIX,
...
-- Demanda do Produto
ISNULL((SELECT  SUM(B3_Q01) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS JAN2020,
ISNULL((SELECT  SUM(B3_Q02) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS FEV2020,
ISNULL((SELECT  SUM(B3_Q03) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS MAR2020,
ISNULL((SELECT  SUM(B3_Q04) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS ABR2020,
ISNULL((SELECT  SUM(B3_Q05) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS MAI2020,
ISNULL((SELECT  SUM(B3_Q06) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS JUN2020,
ISNULL((SELECT  SUM(B3_Q07) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS JUL2020,
ISNULL((SELECT  SUM(B3_Q08) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS AGO2020,
ISNULL((SELECT  SUM(B3_Q09) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS SET2020,
ISNULL((SELECT  SUM(B3_Q10) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS OUT2020,
ISNULL((SELECT  SUM(B3_Q11) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS NOV2020,
ISNULL((SELECT  SUM(B3_Q12) FROM SB3010 WHERE D_E_L_E_T_ = '' AND B3_COD = A.B1_COD ),0) AS DEZ2020,
-- Demanda/Ano
...
FROM SB1010 AS A
	...
	LEFT JOIN SA5010 AS G ON G.D_E_L_E_T_ = '' AND G.A5_PRODUTO = A.B1_COD AND G.A5_FORNECE = E.A2_COD 
WHERE A.D_E_L_E_T_ = '' 
--	AND A.B1_MSBLQL  = '2'
--	AND A.B1_ZPROMOC = 'S'
	AND SUBSTRING(A.B1_GRUPO,1,2) IN ('CF','MP','MT')
--	AND A.B1_COD = '40478'
--	AND A.B1_GRUPO = 'MRUT' 
--	AND A.B1_COD = 'LB012F'   
--	AND A.B1_COD = 'EP199I'   
GROUP BY  A.B1_COD,A.B1_ZIDWEB, A.B1_GRUPO,  A.B1_DESC, A.B1_ZDESRDZ, B5_ALTURA, B5_COMPR, B5_LARG, CONVERT(VARBINARY(8000),A.B1_ZTITWEB),A.B1_ZAPRES1,G.A5_CODPRF,A.B1_UM,D.ZB2_DESFAM,A.B1_UCOM,A.B1_GRTRIB,A.B1_ZPROMOC,A.B1_ZPERDES,A.B1_ZPROWEB,A.B1_ZPDEWEB,A.B1_ZUPSITE,A.B1_PROC,A.B1_LOJPROC,E.A2_NOME,A.B1_CUSTD,A.B1_ZPERCAT,A.B1_ZCODFAM,SUBSTRING(A.B1_GRUPO,1,2),A.B1_MSBLQL,A.B1_ZMIXFL1
ORDER BY 1
