...
SELECT 
A.K_NUMEROCONTRATO,
(SELECT TOP 1 NOME FROM FILIAIS	WHERE HANDLE IN (1)) 				AS FILIAL
,(SELECT TOP 1 NOME	FROM CN_TIPOSCONTRATOS WHERE HANDLE IN (55)) 	AS TIPOCONTRATO_FILTRO
	,CONVERT(DATE, '2023-07-01') AS INICIAL
	,CONVERT(DATE, '2023-07-31')  AS FINAL
	,CASE 
	...
	,C.LOGOTIPO
	,CASE 
		WHEN H.HANDLE IS NOT NULL THEN H.VALOR
		ELSE A.BASECALCULO
		END AS BASECALCULO
	,B.CODIGO
...
SELECT MAX(A.TIPOMOVIMENTO)
	...
	,F.HANDLE HANDLECMS
	,CASE 
		WHEN H.HANDLE IS NOT NULL THEN H.VALOR * (A.PERCENTUAL/100)
		ELSE A.BASECALCULO * (A.PERCENTUAL/100)
		END AS VLR_COMISSAO
FROM FN_DOCUMENTOCOMISSOES A
...
LEFT JOIN K_CN_CONTRATOBENEFICIARIOS H ON H.HANDLE = G.CONTRATOBENEFICIARIO OR H.HANDLE IN (SELECT HANDLE FROM K_CN_CONTRATOBENEFICIARIOS WHERE CONTRATO IN (SELECT HANDLE FROM CN_CONTRATOS WHERE NUMERO = A.K_NUMEROCONTRATO AND K9_TIPOCOBRANCAPARCELASCARNE = 18))
...
WHERE 1 = 1
	AND (A.FILIAL IN (1)OR - 1 IN (1))
	AND (I.TIPOCONTRATO IN (55) OR - 1 IN (55))
	AND (A.AGENTEVENDAS IN (34031) OR - 1 IN (34031))
	AND A.K_NUMEROCONTRATO = 'TFA-00042204-CT'
   	--------------
	AND (
			(
				CASE 
					WHEN E.DATAVENCIMENTO IS NULL
						THEN E1.DATAVENCIMENTO
					ELSE E.DATAVENCIMENTO
					END BETWEEN CONVERT(DATE, '2023-07-01') AND CONVERT(DATE, '2023-07-31') 
				AND (
					CASE 
						WHEN E.HANDLE IS NULL
							THEN E1.HANDLE
...
SELECT PARCELA
	...
	WHERE TIPOMOVIMENTO IN (1, 2, 3, 5, 6, 14, 8)
							...
							AND DATA BETWEEN CONVERT(DATE, '2023-07-01') AND CONVERT(DATE, '2023-07-31') 
						)
					)
			)
		OR (
